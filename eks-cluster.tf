...
resource "aws_eks_cluster" "cvdev" {
 name = "cvdev-cluster"
...
#The last bit of code is to create the worker nodes. For testing purposes, 
#use just one worker node in the scaling_config configuration. 
#In production, follow best practices and use at least three worker nodes.


resource "aws_eks_node_group" "worker-node-group" {
  cluster_name  = aws_eks_cluster.cvdev.name
  node_group_name = "cvdev-workernodes"
